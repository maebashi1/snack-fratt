...
デザインは Club Original（https://original.value-box.com/）のレイアウトを下敷きにしています。画面上部に固定されたセル型のグローバルナビ、斜めに切り取ったフォトバンド、金額を大きな数字で右寄せする料金表といった構成をそのまま踏襲しています。配色は白ベースにオレンジ（`#ff7a1a`）を効かせ、写真のバンドとページヘッドだけを暗くしてメリハリをつけています。
...
`logo.png` は白背景用に作られたロゴなので、白ベースのヘッダーにそのまま乗ります。特別な処理は入れていません。
...
:root{
  ...
  --orange-light: #ffa04d;  /* 明るいオレンジ */
  --orange-deep:  #d95d05;  /* 濃いオレンジ（文字やホバーに使用） */
  --bg:           #ffffff;  /* 背景の白 */
  --bg-alt:       #fbf7f3;  /* 1段落とした背景 */
  --ink:          #241c16;  /* 写真バンドやページヘッドの下地 */
  --text:         #2b221b;  /* 本文の文字色 */
  ...
}
```
...
「前橋 スナック」「群馬 スナック」といった検索で見つけてもらうため、サイト側は次の状態にしてあります。

各ページのタイトルと説明文には地域名（群馬県・前橋市・千代田町）と業種、料金や営業時間といった具体的な情報を入れてあります。トップページには `<h1>` を1つだけ置き、見出しは h1 → h2 → h3 の順に並べています。トップページには店舗情報（LocalBusiness）とよくあるご質問（FAQPage）の構造化データを、下層ページにはパンくずの構造化データを埋め込んであります。営業時間や料金を変えたときは、本文だけでなく `index.html` の `<head>` にある構造化データも合わせて直してください。内容がずれていると Google に無視されます。

画像はすべて横1800pxに縮小して圧縮済みです（合計16MB→1.5MB）。表示速度は検索順位に影響するので、写真を差し替えるときも1枚300KB以下を目安にしてください。あわせて全画像に `width` / `height` と遅延読み込みを指定し、スクロール時のガタつきを抑えています。

`sitemap.xml` は手動更新です。ページを増やしたら `<url>` を追記し、内容を大きく変えたら `<lastmod>` の日付を更新して、Search Console から再送信してください。

### サイト側では解決できないこと

ここから先は、コードをどういじっても順位に効かない部分です。効果の大きい順に3つあります。

ひとつ目が **Google ビジネスプロフィール** です。「前橋 スナック」のような地域＋業種の検索では、地図付きのローカル枠が検索結果の上部を占めます。ここに載るかどうかを決めるのはビジネスプロフィールの登録内容であって、ウェブサイトではありません。未登録なら <https://www.google.com/business/> から登録し、営業時間・電話番号・写真・カテゴリ（バー／スナック）を埋めて、ウェブサイト欄にこのサイトのURLを入れてください。口コミが増えるほど有利になります。実質これが一番効きます。
